...
export const PinContainer = ({
  ...
  const [transform, setTransform] = useState(
    "translate(-50%,-50%) rotateX(0deg) scale(0.85)"
  );
  ...
  const onMouseLeave = () => {
    setTransform("translate(-50%,-50%) rotateX(0deg) scale(0.85)");
  };

  return (
    <div
      className={cn(
        "relative group/pin z-50 cursor-pointer h-[240px] w-[220px]",
        containerClassName
      )}
...
export const PinPerspective = ({ title, href }) => {
  return (
    <motion.div className="pointer-events-none w-full h-full flex items-center justify-center opacity-0 group-hover/pin:opacity-100 z-50 transition duration-500">
      <div className="w-full h-full -mt-7 flex-none  inset-0">
        ...
        <>
          ...
          <motion.div className="absolute right-1/2 translate-x-[1.5px] bottom-1/2 bg-cyan-600 translate-y-[14px] w-[4px] h-[4px] rounded-full z-40" />

          <motion.div className="absolute right-1/2 translate-x-[-1.5px] bottom-1/2 bg-cyan-700 translate-y-[14px] w-[2px] h-[2px] rounded-full z-40" />
        </>
      </div>
    </motion.div>
  );
